...
export default function ModelDetailPage() {
  ...
  useEffect(() => { setModel(models.find((m) => m.id === Number(id)) ?? null); }, [models, id]);
  useEffect(() => { if (model && address) checkAccess(model.id).then(setHasAccess); }, [model, address, checkAccess]);
      ...
      <div className="detail-layout">
        <div className="detail-main">
          ...
          <div className="ipfs-block">
            ...
            <a href={`https://ipfs.io/ipfs/${model.ipfsHash}`} target="_blank" rel="noreferrer" className="ipfs-link">
          ...
          <div className="purchase-card">
            ...
            {hasAccess ? (
              <div className="access-granted">
                ...
                <a href={`https://ipfs.io/ipfs/${model.ipfsHash}`} target="_blank" rel="noreferrer" className="btn btn--primary">Download ↗</a>
              </div>
            ) : (
              <button className="btn btn--primary btn--full" onClick={handlePurchase} disabled={isPurchasing}>
                {!address ? "Connect Wallet to Purchase" : isPurchasing ? "Processing…" : `Purchase for ${model.price} ETH`}
              </button>
            )}
            <TxBadge tx={tx} />
